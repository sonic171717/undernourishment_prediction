...
#function to collect the satellite data and produce a yearly country specific average laihv value
def laihv_collection(country, year):
    ...
    aggregate_pixel_values = (pixel_values.aggregate_array("leaf_area_high_vegetation").getInfo())
...
#iterating through years and countries to get each laihv value
i = 0
while i < len(countries):
    year = 2001
    while year <= 2020:
        result = laihv_collection(country = countries[i], year = year)
        laihv_values.append(result)
...
df_laihv = pd.DataFrame()
df_laihv["GAUL"] = country_index
df_laihv["Year"] = time_index
df_laihv["LAI high veg"] = laihv_values

#saving the results
df_laihv.to_csv(path + (r"\laihv_data.csv"))
